...
res_path = f"preds_gpt/{model_name}/"
result = dict()
score_df = dict()
score_df['resp_len'] = []
score_df['id'] = []
...
json.dump(result, open(f"preds_gpt/{model_name}/judge_result.json","w"), ensure_ascii=False, indent=4)

pd.DataFrame(result).transpose().to_csv(f"preds_gpt/{model_name}/judge_result.csv")
...
for dim in dims:
    ...
    fig.write_html(f"preds_gpt/{model_name}/judge_scores_hist_{dim}.html")

try:
    with pd.ExcelWriter(f"preds_gpt/LW_score_data.xlsx",mode='a',if_sheet_exists='replace') as writer:
        score_df.to_excel(writer,sheet_name=args.model)
except FileNotFoundError:
    with pd.ExcelWriter(f"preds_gpt/LW_score_data.xlsx",mode='w') as writer:
        score_df.to_excel(writer,sheet_name=args.model)
