...
class DynamicCache(Cache):
    ...
    def cleanup(
        ...
        # print(f"Picking {self.key_cache[layer_idx].shape[2]} KVs\n")
            # gc.collect()
        if layer_idx==0:
...
class OffloadedCache(DynamicCache):
    ...
    def evict_previous_layer(self, layer_idx: int):
        ...
        # omit query cache from prefetch
        ...
        if self.attn_cache[prev_layer_idx]!=[]:    
            ...
            # if self.cleaned[prev_layer_idx]:
            #     torch.cuda.empty_cache()
            #     gc.collect()
            #     self.cleaned[prev_layer_idx] = False
            # print(f"evicted layer: {prev_layer_idx}")
    ...
    def __getitem__(self, layer_idx: int) -> List[Tuple[torch.Tensor]]:
        ...
        if layer_idx < len(self):
            ...
            #     query_tensor = query_tensor.index_select(0, self.beam_idx)
        ...
        # so we explicitly check for the empty list
            ...
            if query_states!=None:
                if self.query_cache[layer_idx].shape[-2]==self.win_size:
                    self.query_cache[layer_idx] = torch.roll(self.query_cache[layer_idx],shifts=-1,dims=2) #shift up by 1 position on the 2nd dim
                    self.query_cache[layer_idx][:,:,-1:,:] = query_states[..., -1:, :]
                else:
                    self.query_cache[layer_idx] = torch.cat([query_tensor, query_states], dim=-2)

        if layer_idx==0 and query_states!=None:
            # import pdb; pdb.set_trace()
            BS,NH,LEN,DIM = self.key_cache[layer_idx].shape
            if self.key_cache[layer_idx] != []: self.cache_size['key'] = min(self.cache_size['key'],self.key_cache[layer_idx].shape[2])#len(self.key_cache) * self.key_cache[0].shape[0] * self.key_cache[0].shape[1] * self.key_cache[0].shape[2] * self.key_cache[0].shape[3])
            if self.value_cache[layer_idx] != []: self.cache_size['value'] = min(self.cache_size['value'], self.value_cache[layer_idx].shape[2]) #len(self.value_cache) * self.value_cache[0].shape[0] * self.value_cache[0].shape[1] * self.value_cache[0].shape[2] * self.value_cache[0].shape[3])
            if self.query_cache[layer_idx] != []: self.cache_size['query'] = min(self.cache_size['query'], self.query_cache[layer_idx].shape[2]) #len(self.attn_cache) * self.attn_cache[0].shape[0] * self.attn_cache[0].shape[1] * self.attn_cache[0].shape[2] * self.attn_cache[0].shape[3])
            if not self.prefill:
                self.cache_size['len'] = LEN
