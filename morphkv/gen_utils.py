...
class GenerationMixin:
    ...
    def _prepare_cache_for_generation(
        ...
        # keeps copying the cache thus using much more memory
            ...
            model_kwargs[cache_name] = (
                OffloadedCache(num_hidden_layers)
                if not requires_cross_attention_cache
                else EncoderDecoderCache(OffloadedCache(num_hidden_layers), OffloadedCache(num_hidden_layers))
            )
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            if not self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device, cur_len=cur_len, max_length=max_length):
                torch.cuda.empty_cache()
                gc.collect() 
                print(f"........Input size: {outputs.past_key_values.input_size} KV Cache size: {outputs.past_key_values.cache_size}...........GPU Max Reserved: {torch.cuda.max_memory_reserved(input_ids.device)/1000000000} GB\n")
                LG.info(f"........Input size: {outputs.past_key_values.input_size} KV Cache size: {outputs.past_key_values.cache_size}...........GPU Max Reserved: {torch.cuda.max_memory_reserved(input_ids.device)/1000000000} GB\n")
            print(f"........Input size: {outputs.past_key_values.input_size} KV Cache size: {outputs.past_key_values.cache_size}...........GPU Max Reserved: {torch.cuda.max_memory_reserved(input_ids.device)/1000000000} GB\n")
            del outputs
